...
import com.nocountry.backend.handler.Exceptions.LoginException;
import com.nocountry.backend.handler.Exceptions.OverlappedBookingException;
import com.nocountry.backend.handler.Exceptions.RegisterException;
...
public class RestApiErrorHandler {
        ...
        public ResponseEntity<Error> LoginException(HttpServletRequest request,
                ...
                Error error = ErrorUtils
                                .createError(
                                                String.format("%s %s", ErrorCode.LOGIN_BAD_CREDENTIALS.getErrMsgKey(),
                                                                ex.getMessage()),
                                                ex.getErrorCode(),
        ...
        public ResponseEntity<Error> handleRegisterException(HttpServletRequest request,
                ...
                Error error = ErrorUtils
                                .createError(
                                                String.format("%s %s", ErrorCode.USER_REGISTER.getErrMsgKey(),
                                                                ex.getMessage()),
                                                ex.getErrorCode(),
        ...
        public ResponseEntity<Error> handleGenericNotFoundException(HttpServletRequest request,
                ...
                Error error = ErrorUtils
                                .createError(
                                                String.format("%s %s", ErrorCode.ENTITY_NOT_FOUND.getErrMsgKey(),
                                                                ex.getMessage()),
                                                ex.getErrorCode(),
        ...
        public ResponseEntity<Error> handleCarNotFoundException(HttpServletRequest request,
                        CarNotFoundException ex, Locale locale) {
                Error error = ErrorUtils
                                .createError(
                                                String.format("%s %s", ErrorCode.CAR_NOT_FOUND.getErrMsgKey(),
                                                                ex.getMessage()),
                                                ex.getErrorCode(),
                                                HttpStatus.NOT_FOUND.value())
                                .setUrl(request.getRequestURL().toString())
        ...
        public ResponseEntity<Error> handleOverlappedBookingException(HttpServletRequest request,
                ...
                Error error = ErrorUtils
                                .createError(
                                                String.format("%s %s", ErrorCode.OVERLAPPED_BOOKING.getErrMsgKey(),
                                                                ex.getMessage()),
                                                ex.getErrorCode(),
                                                HttpStatus.NOT_FOUND.value())
                                .setUrl(request.getRequestURL().toString())
